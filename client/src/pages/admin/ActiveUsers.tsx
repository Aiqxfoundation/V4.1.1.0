...
import { 
  ArrowLeft, UserCheck, Search, Settings, Calendar, 
  Hash, ChevronLeft, ChevronRight, Activity, Mail
} from "lucide-react";
...
function ActiveUsers() {
  ...
  const activeUsers = useMemo(() => 
    allUsers.filter(u => !u.isFrozen && !u.isBanned && u.miningActive),
    [allUsers]
  );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400">
                {isLoading ? "Loading..." : `${activeUsers.length} users currently active and mining`}
              </p>
            </div>
          </div>
        </div>
      ...
      <Card className="bg-zinc-900 border-zinc-800">
        <CardHeader>
          <CardTitle className="text-xl text-white">
            {searchQuery 
              ? `Found ${searchedUsers.length} active users matching "${searchQuery}"`
              : `All Active Users (${sortedUsers.length} total)`
            }
          </CardTitle>
        </CardHeader>
        <CardContent>
                      ...
                      >
                        ...
                        <TableCell className="hidden md:table-cell">
                          {user.email ? (
                            <div className="flex items-center gap-2 text-gray-300">
                              ...
                              <span className="text-sm" title={user.email}>{user.email}</span>
                            </div>
                          ) : (
                        ...
                        <TableCell className="text-right">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={useCallback(() => navigateToProfile(user.id), [user.id, navigateToProfile])}
