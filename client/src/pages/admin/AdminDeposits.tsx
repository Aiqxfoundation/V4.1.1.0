...
export default function AdminDeposits() {
  ...
  const [approveData, setApproveData] = useState({ actualAmount: "", txHash: "" });

  // Fetch all deposits
  const { data: allDeposits = [], isLoading, refetch } = useQuery<Deposit[]>({
  ...
  const filteredDeposits = allDeposits.filter(deposit => {
    const matchesSearch = 
      ...
      false;
    const matchesStatus = deposit.status === statusFilter;
  ...
  const approveDepositMutation = useMutation({
    mutationFn: async ({ id, actualAmount, txHash }: { id: string; actualAmount?: string; txHash?: string }) => {
      const res = await apiRequest("PATCH", `/api/deposits/${id}/approve`, { actualAmount, txHash });
      return res.json();
    },
    onSuccess: () => {
      toast({ 
      ...
      setApproveData({ actualAmount: "", txHash: "" });
    },
    onError: () => {
      toast({ 
        title: "Error", 
  ...
  const handleApproveDeposit = (deposit: Deposit) => {
    ...
    setApproveData({ actualAmount: deposit.amount, txHash: "" });
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <p className="font-medium text-sm">{deposit.user?.username || 'Unknown'}</p>
                        <p className="text-green-400 font-bold">
                          {deposit.currency === 'USDT' ? '$' : ''}
                          {parseFloat(deposit.amount).toFixed(deposit.currency === 'USDT' ? 2 : 4)}
                          {deposit.currency !== 'USDT' ? ' ' + deposit.currency : ''}
                        </p>
                        ...
                        {deposit.status === 'pending' && (
                          <div className="mt-2 space-x-1">
                            <Button 
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleApproveDeposit(deposit);
                              }}
                              data-testid={`button-quick-approve-${deposit.id}`}
                            >
                              ✓
                            </Button>
                            <Button 
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleReject(deposit.id);
                              }}
                              data-testid={`button-quick-reject-${deposit.id}`}
                            >
                              ✗
                            </Button>
                          </div>
              ...
              <div className="hidden sm:block w-full overflow-x-auto">
                ...
                <TableBody>
                  {filteredDeposits.map((deposit) => (
                    ...
                    >
                      <TableCell className="font-medium text-[11px] px-1 py-2">{deposit.user?.username || 'N/A'}</TableCell>
                      <TableCell className="text-[11px] px-1 py-2">
                        <span className="text-green-500 font-semibold">
                          <span className="sm:hidden">
                            {deposit.currency === 'USDT' ? '$' : ''}
                            {deposit.currency === 'USDT' ? Math.round(parseFloat(deposit.amount)) : parseFloat(deposit.amount).toFixed(4)}
                          </span>
                          <span className="hidden sm:inline">
                            {deposit.currency === 'USDT' ? '$' : ''}
                            {parseFloat(deposit.amount).toFixed(deposit.currency === 'USDT' ? 2 : 8)}
                          </span>
                        </span>
                      </TableCell>
                        ...
                        {deposit.txHash ? (
                          <div className="flex items-center gap-1">
                            <span className="text-[10px] text-gray-400">
                              <span className="sm:hidden">{deposit.txHash.slice(0, 6)}...</span>
                              <span className="hidden sm:inline max-w-[100px] truncate">{deposit.txHash}</span>
                            </span>
                            <Button
                              size="icon"
                              variant="ghost"
                              ...
                              data-testid={`button-copy-tx-${deposit.id}`}
                            >
                        ...
                        {deposit.address ? (
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              ...
                              data-testid={`button-copy-address-${deposit.id}`}
                            >
                              <Copy className="h-3 w-3" />
                            </Button>
                          </div>
                      ...
                      <TableCell className="text-[11px] px-1 py-2 text-gray-400">
                        <span className="sm:hidden">{new Date(deposit.createdAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</span>
                        <span className="hidden sm:inline">{new Date(deposit.createdAt).toLocaleString()}</span>
                      </TableCell>
                      <TableCell className="text-[11px] px-1 py-2">{getStatusBadge(deposit.status)}</TableCell>
                      <TableCell className="text-[11px] px-1 py-2">
                        {deposit.status === 'pending' ? (
      ...
      <Dialog open={showApproveDialog} onOpenChange={setShowApproveDialog}>
        <DialogContent className="bg-zinc-900 border-zinc-800">
          <DialogHeader>
            ...
            <DialogDescription>
              Confirm the deposit details and optionally adjust the amount
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Input
                value={selectedDeposit?.user?.username || ''}
                disabled
            ...
            <div>
              ...
              <Input
                value={`${selectedDeposit?.amount} ${selectedDeposit?.currency}`}
                disabled
                className="bg-zinc-800 border-zinc-700"
              />
            </div>
            <div>
              <Label>Actual Amount (if different)</Label>
              <Input
                value={approveData.actualAmount}
                onChange={(e) => setApproveData(prev => ({ ...prev, actualAmount: e.target.value }))}
              ...
              onClick={() => {
                if (selectedDeposit) {
                  approveDepositMutation.mutate({
                    ...
                    actualAmount: approveData.actualAmount || undefined,
      ...
      <Dialog open={!!selectedTransaction} onOpenChange={() => setSelectedTransaction(null)}>
        <DialogContent className="bg-zinc-900 border-zinc-800 max-w-md">
          ...
          <div className="space-y-3">
            ...
            <div>
              <p className="text-[10px] text-gray-400">Deposit Address</p>
              <div className="flex items-center gap-2">
                ...
                {selectedTransaction?.address && (
                  <Button 
                    size="sm" 
                    variant="ghost" 
                    onClick={() => copyToClipboard(selectedTransaction.address!, 'Deposit address')}
          ...
          {selectedTransaction?.status === 'pending' && (
            <DialogFooter className="mt-4">
              <Button 
                ...
                onClick={() => {
                  setSelectedDeposit(selectedTransaction);
